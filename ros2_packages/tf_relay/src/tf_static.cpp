...
: Node("tf_relay_tf_static", options)
{
  ...
  if (robot_namespaces_.empty()) {
    RCLCPP_WARN(this->get_logger(), "No robot namespaces provided for static TF. Nothing will be relayed.");
    return;
  }

  for (auto & ns : robot_namespaces_) {
    // Input topic: ns + "/tf_static"
    // Output topic: ns + "/relayed/tf_static"
